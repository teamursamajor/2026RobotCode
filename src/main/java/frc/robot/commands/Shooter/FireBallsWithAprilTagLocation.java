...
public class FireBallsWithAprilTagLocation extends Command {
    ...
    public FireBallsWithAprilTagLocation(ShooterSubsystem shooterSubsystem, double desiredDistance,
    ...
    public double ShootVelocityToHitPoint(double x, double y) {
        double velocity = 1;
        double gravity = 9.8;
        velocity = Math.sqrt(((gravity * Math.pow(x, 2) * Math.pow(Math.tan(gravity), 2)) + gravity * Math.pow(x, 2))
                / (2 * (-y + (x * Math.tan(shooterAngle)))));
        return velocity;
    }
